...
describe('playwrightWeb', () => {
  ...
  it('sets browser type and device', async () => {
    const { world, vstep, steppers } = await getTestEnv([wp], 'using firefox.Pixel 5 browser', getDefaultWorld().world);
  ...
  it('fails setting browser type and device', async () => {
    const { world, vstep } = await getTestEnv([wp], 'using nonexistant browser', getDefaultWorld().world);
