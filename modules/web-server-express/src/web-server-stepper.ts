...
const WebServerStepper: IExtensionConstructor = class WebServerStepper
  implements IStepper, IHasOptions
{
  ...
  constructor(world: TWorld) {
    this.world = world;
    this.webserver = new ServerExpress(
      this.world.logger,
      [process.cwd(), "files"].join("/")
    );
  ...
  static async checkListener(options: TOptions, webserver: IWebServer) {
    const port = options[
      `HAIBUN_O_${WEBSERVER_STEPPER.toUpperCase()}_PORT`
    ] as number;
  ...
  steps = {
    serveFiles: {
      ...
      action: async ({ loc }: TKeyString) => {
        await WebServerStepper.checkListener(
          this.world.options,
          this.world.runtime[WEBSERVER]
        );
