...
import server, {
  ...
  WEBSERVER,
  WEBSERVER_STEPPER,
} from "./web-server-stepper";
import {
  IExtensionConstructor,
  IStepper,
  TKeyString,
  TWorld,
} from "@haibun/core/src/lib/defs";

describe("route mount", () => {
  it("mounts a route", async () => {
    const TestRoute: IExtensionConstructor = class TestRoute
      implements IStepper
    {
      world: TWorld;
      ...
      steps = {
        addRoute: {
          ...
          action: async ({ loc }: TKeyString) => {
            ...
            const webserver: IWebServer = getFromRuntime(
              this.world.runtime,
              WEBSERVER
            );

            const checkListener = getFromRuntime<ICheckListener>(
              this.world.runtime,
              CHECK_LISTENER
            );
