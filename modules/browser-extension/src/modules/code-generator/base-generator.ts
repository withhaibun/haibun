...
export default abstract class BaseGenerator {
  ...
  abstract generate(events: any): string
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      switch (action) {
        ...
        case headlessActions.GOTO:
          this._blocks.push(this._handleGoto(href/*,_frameId*/))
          break
        ...
        case headlessActions.NAVIGATION:
          this._blocks.push(this._handleWaitForNavigation(JSON.stringify(events[i])))
  ...
  _setFrames(frameId: any, frameUrl: any) {
  ...
  _handleKeyDown(selector: any, value: any) {
    const block = new Block(this._frameId)
    block.addLine({
...
await element${this._screenshotCounter}.screenshot({ path: 'screenshot_${this._screenshotCounter}.png' })`,
  ...
  _handleWaitForNavigation(href: any) {
    const block = new Block(this._frameId)
    if (this._options.waitForNavigation) {
      block.addLine({
        type: headlessActions.NAVIGATION,
        value: `await navigationPromise(${href})`,
      })
    }
    return block
  }
