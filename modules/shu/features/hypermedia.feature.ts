...
const { graphQuery, createEdge, exportGraphAsJsonLd } = withAction(new TutorialGraphStepper());
const { serveShuApp } = withAction(new ShuStepper());
const { waitFor, click, gotoPage, reloadPage } = withAction(new WebPlaywright());
const { feature, scenario } = withAction(new Haibun());
const host = "http://localhost:8223";
...
export const features: TKirejiExport = {
	"Hypermedia Tutorial: Rels, Edges, and HATEOAS": [
		...
		scenario({ scenario: "Discovering Incoming Relationships" }),
		"Papers have edges linking back to Researchers via attributedTo.",
		"The shu UI also queries incoming edges: 'What links to this Paper?'",
		"This implements HATEOAS discovery from the reverse direction.",
		"",
		"In the Paper column, shu shows incomingCount (e.g., '2 citations').",
		"Clicking reveals which Papers cite this one via inReplyTo rels.",
