 ...
 *
 * A scene draws frames, and what a frame costs the renderer is not visible from the page: the work is done in another
 * process, and the main thread spends half a millisecond where a software rasterizer spends sixteen. The scene
 * measures the cost with a fence and records it here, and records the regulation it applies to itself on that
 * measurement, so a run can observe a page rest its decorative motion and read the cost that decided it.
 */
import { z } from "zod";
 ...
 *  fence placed after the draw. The attributes state how much was drawn, so a cost can be read against a size. */
export const GRAPH_FRAME_BLIP = "haibun.shu.graph.frame";

/** A scene regulated its own decorative motion: `signal` states whether the breath rested (over budget) or resumed
 *  (within budget), `value` is the median frame cost compared with the budget, and `share` the breath's share of wall
 *  time. */
