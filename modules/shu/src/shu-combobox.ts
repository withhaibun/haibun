...
export class ShuCombobox extends ShuElement<typeof ComboboxSchema> {
	...
	constructor() {
		super(ComboboxSchema, { value: "", options: [], placeholder: "", filterText: "", open: false });
	}
	...
	setValue(value: string): void {
		...
		this.state = { ...this.state, value, filterText: match?.label ?? value, open: false };
...
<input type="text" class="combo-input" placeholder="${escAttr(this.state.placeholder)}" value="${escAttr(this.state.filterText)}" autocomplete="off"${testId ? ` data-testid="${escAttr(testId)}"` : ""} />`;
	...
	private renderList(): void {
		...
		if (items.length > 0) {
			...
			Object.assign(li.style, { ...LI_STYLE, color: "#999", fontStyle: "italic", cursor: "default" });
			ul.appendChild(li);
		}
		...
		if (this._focusIndex >= 0 && this._focusIndex < ul.children.length) {
			(ul.children[this._focusIndex] as HTMLElement).scrollIntoView({ block: "nearest" });
		}
	}
	...
	private bindEvents(): void {
		...
		input.addEventListener("input", () => {
			this._focusIndex = -1;
			this.state = { ...this.state, filterText: input.value, value: "", open: true };
			this.renderList();
		});
	...
	private pick(option: TComboboxOption): void {
		...
		this._focusIndex = -1;
		this.state = { ...this.state, value: option.value, filterText: option.label, open: false };
