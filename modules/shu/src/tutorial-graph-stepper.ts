...
class TutorialGraphStore {
	...
	private seedData() {
		...
		this.edges = [
			{ id: "edge-alice-authored-semantic", fromLabel: "Researcher", fromId: "researcher-alice", rel: "attributedTo", toLabel: "Paper", toId: "paper-semantic-web" },
			{ id: "edge-bob-authored-hypermedia", fromLabel: "Researcher", fromId: "researcher-bob", rel: "attributedTo", toLabel: "Paper", toId: "paper-hypermedia" },
			{ id: "edge-alice-coauthor-jsonld", fromLabel: "Researcher", fromId: "researcher-alice", rel: "attributedTo", toLabel: "Paper", toId: "paper-jsonld" },
			{ id: "edge-semantic-references-hypermedia", fromLabel: "Paper", fromId: "paper-semantic-web", rel: "inReplyTo", toLabel: "Paper", toId: "paper-hypermedia" },
			{ id: "edge-hypermedia-references-jsonld", fromLabel: "Paper", fromId: "paper-hypermedia", rel: "inReplyTo", toLabel: "Paper", toId: "paper-jsonld" },
		];
	}
...
export default class TutorialGraphStepper extends AStepper {
		...
		getConcerns: (): IStepperConcerns => ({
			domains: [
				...
				},
				{
					...
					schema: VertexSchema,
					meta: {
						vertexLabel: "Researcher", id: "id",
						properties: { id: "identifier", name: "name", context: "context", published: "published" },
						edges: { authored: { rel: "attributedTo", target: "Paper" } },
					},
				},
				{
					...
					schema: VertexSchema,
					meta: {
						vertexLabel: "Paper", id: "id",
						properties: { id: "identifier", name: "name", content: "content", published: "published", updated: "updated" },
						edges: { references: { rel: "inReplyTo", target: "Paper" }, author: { rel: "attributedTo", target: "Researcher" } },
					},
				},
			],
		}),
	...
	steps: TStepperSteps = {
		...
		getIncomingEdges: {
			...
			action: async ({ label, id, limit = 100, offset = 0 }: { label: string; id: string; limit?: number; offset?: number }) => {
				try {
		...
		createEdge: {
			...
			action: async ({ fromLabel, fromId, rel, toLabel, toId }: { fromLabel: string; fromId: string; rel: string; toLabel: string; toId: string }) => {
				try {
