...
export class ShuResultTable extends ShuElement<typeof ResultTableSchema> {
	...
	setPagination(total: number, limit: number, offset: number): void {
		this.state = { ...this.state, total, limit, offset, paginated: total > limit };
	...
	private emitPageChange(offset: number): void {
		this.dispatchEvent(new CustomEvent("page-change", { detail: { offset }, bubbles: true, composed: true }));
	}
	...
	private bindEvents(): void {
		...
		this.shadowRoot?.querySelectorAll("th[data-field]").forEach((th) => {
			th.addEventListener("click", () => {
				...
				this.dispatchEvent(new CustomEvent("sort-change", { detail: { field, order: newOrder }, bubbles: true, composed: true }));
			});
		});
		...
		if (this.state.selectable) {
			...
			this.shadowRoot?.querySelector(".results-wrapper")?.addEventListener("click", (e) => {
				...
				this.deselectAll();
				this.dispatchEvent(
					new CustomEvent("row-click", { detail: { vertexId: null, deselect: true }, bubbles: true, composed: true }),
				);
			});
