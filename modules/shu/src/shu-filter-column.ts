...
export class ShuFilterColumn extends ShuElement<typeof FilterColumnSchema> {
	...
	async openFiltered(property: string, value: string, label: string = defaultLabel()): Promise<void> {
		this.state = { ...this.state, property, value, vertexLabel: label, loading: true, error: undefined };
	...
	async openProperty(property: string, label: string = defaultLabel()): Promise<void> {
		this.state = { ...this.state, property, vertexLabel: label, loading: true, error: undefined };
		this.showSpinner("Fetching...");
		await this.fetchResults({
			label,
			filters: [],
	...
	async openIncoming(targetId: string, targetLabel: string): Promise<void> {
		this.state = { ...this.state, vertexLabel: targetLabel, property: "linksTo", value: targetId, loading: true, error: undefined };
		this.showSpinner("Fetching...");
	...
	private async fetchIncoming(label: string, id: string, limit: number, offset: number): Promise<void> {
		try {
			...
			const data = await client.rpc<{ edges: Array<{ type: string; target: VertexData }>; total: number }>(
				requireStep("getIncomingEdges"),
				{ label, id, limit, offset },
			);
			...
			if (vid) {
				this.dispatchEvent(
					new CustomEvent("column-open", {
						detail: { subject: vid, label: rowLabel || this.state.vertexLabel || defaultLabel() },
						bubbles: true,
						composed: true,
					}),
				);
	...
	private showResults(): void {
		...
		if (this.resultTable) {
			...
			this.resultTable.updateState({ displayMode: "full", fixedProperty: property });
