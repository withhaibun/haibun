...
export class ShuColumnBrowser extends HTMLElement {
	...
	async openColumn(id: string, afterIndex?: number, label?: string): Promise<void> {
		...
		try {
			...
			if (!stepDesc?.method) {
				this.columns[colIndex] = { label: id, vertexLabel, error: `No step for ${vertexLabel}`, query };
				this.render();
				return;
			}
			const data = await client.rpc<{ vertex: VertexData; edges: EdgeData[] }>(stepDesc.method, { id });
			this.columns[colIndex] = { label: id, vertexLabel, vertex: data.vertex, edges: data.edges ?? [], query };
			this.render();
		} catch (err) {
			this.columns[colIndex] = { label: id, vertexLabel, error: errMsg(err), query };
			this.render();
		}
	}
	...
	async openQueryColumn(uri: string, afterIndex?: number): Promise<void> {
		...
		const label = uri;
		const query: ColumnQuery = { vertexId: payload.vertexId as string | undefined, label: payload.label as string | undefined };
	...
	async openFilteredColumn(property: string, value: string, label: string, afterIndex?: number): Promise<void> {
		...
		try {
			...
			this.columns[colIndex] = { label: colLabel, vertexLabel: label, results: data.vertices, query };
			this.render();
		} catch (err) {
	...
	async openPropertyColumn(property: string, label: string, afterIndex?: number): Promise<void> {
		...
		try {
			...
			const client = SseClient.for("");
			const data = await client.rpc<{ vertices: VertexData[]; total: number }>(requireStep("graphQuery"), {
				query: { label, filters: [], sortBy: property, sortOrder: "asc", limit: 50, offset: 0, accessLevel: "private" },
			});
			this.columns[colIndex] = { label: colLabel, vertexLabel: label, results: data.vertices, query };
			this.render();
		} catch (err) {
		...
		if (currentCol) {
			this.dispatchEvent(
				new CustomEvent("context-change", {
					detail: { patterns: [{ s: currentCol.label }], accessLevel: "private" },
					bubbles: true,
					composed: true,
				}),
			);
