...
export class ShuColumnPane extends ShuElement<typeof ColumnPaneSchema> {
	constructor() {
		super(ColumnPaneSchema, { label: "", active: false, closable: true, columnType: "query" });
	}
	...
	attributeChangedCallback(name: string, _old: string | null, val: string | null): void {
		...
		if (name === "column-type")
			this.state = { ...this.state, columnType: (val as "query" | "entity" | "filter" | "property") || "query" };
	...
	protected render(): void {
		...
		this.shadowRoot.querySelector(".pane-close")?.addEventListener("click", (e) => {
			e.stopPropagation();
			this.dispatchEvent(new CustomEvent("column-close", { bubbles: true, composed: true }));
		});
		...
		if (handle) {
			...
			const onPointerUp = () => {
				...
				this.dispatchEvent(
					new CustomEvent("column-resize", { detail: { width: this.state.width }, bubbles: true, composed: true }),
				);
			};
		...
		this.shadowRoot.querySelector(".pane-content")?.addEventListener("click", () => {
			if (!this.state.active) {
				this.dispatchEvent(new CustomEvent("column-activate", { bubbles: true, composed: true }));
			}
		});
		...
		this.shadowRoot.querySelector(".pane-header")?.addEventListener("click", () => {
			if (this.isCollapsed) {
				this.dispatchEvent(new CustomEvent("column-expand", { bubbles: true, composed: true }));
			}
		});
