...
import type { ShuSpinner } from "./components/shu-spinner.js";
import type { ShuCombobox } from "./components/shu-combobox.js";
...
export class ShuActionsBar extends ShuElement<typeof ActionsBarSchema> {
	...
	connectedCallback(): void {
		...
		this._unsubscribeSync = client.onEvent(
			(event) => {
				this._syncEventSeq++;
				this.dispatchEvent(new CustomEvent("sync-available", { detail: event, bubbles: true, composed: true }));
			},
	...
	private async loadModels(): Promise<void> {
		try {
			...
			const client = SseClient.for("");
			const data = await client.rpc<{ models: Array<{ filename: string; contextSize: number }> }>(requireStep("showModels"));
			if (data.models) {
	...
	private bindEvents(): void {
				...
				if (Math.abs(y - startY) > 5) {
					...
					if (!rafPending) {
						...
						requestAnimationFrame(() => {
							rafPending = false;
							this.dispatchEvent(new CustomEvent("resize-drag", { bubbles: true, composed: true, detail: { clientY: y } }));
						});
					}
				}
			};
		...
		stepCombo?.addEventListener("combo-change", ((e: CustomEvent) => {
			...
			if (!stepName) return;
			if (stepName === this._selectedStep) return;
			...
			if (lastCaller && !lastCaller.executed && lastCaller.reset) {
				...
				const caller = document.createElement("step-caller") as HTMLElement;
	...
	private async handleSave(): Promise<void> {
		...
		try {
			...
			await client.rpc(requireStep("saveSummary"), {
				...
				conditions: { conditions: this._filterConditions, label: this._selectedLabel, textQuery: this._textSearch },
...
const STYLES = `
	...
	step-caller { display: block; padding: 4px 6px; margin: 2px 6px; background: #f5f5f5; border-radius: 3px; }
