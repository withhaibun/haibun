...
async function aRunOf(steps: number, saying: string = "info"): Promise<QuadStore> {
	...
	for (let i = 0; i < steps; i++) {
		await store.upsertIndividual(SEQ_PATH_LABEL, { id: `0.${i}`, stepText: `step ${i}`, actionStatus: "passed", generatedAtTime: iso(1000 + i * 2), endedAtTime: iso(1001 + i * 2) });
		await store.upsertIndividual(LOG_MESSAGE_LABEL, { id: `0.${i}@said`, message: `said ${i}`, level: saying, generatedAtTime: iso(1001 + i * 2), isPartOf: `0.${i}` });
	}
	return store;
}
...
describe("the window of a run a reader is looking at", () => {
	...
	it("holds the steps and what they said, oldest first, and says what it spans", async () => {
		...
		expect(window.rows[0]).toMatchObject({ kind: "step", step: "0.0", status: "passed", endedAt: 1001 });
		expect(window.rows[1]).toMatchObject({ kind: "said", step: "0.0", level: "info" });
	});
