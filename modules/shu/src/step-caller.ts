...
type InputProperty = { type?: string; description?: string; enum?: string[]; [key: string]: unknown };
...
export class StepCaller extends HTMLElement {
	...
	private async callStep(formValues: Record<string, string> = {}): Promise<void> {
		...
		try {
			...
			this.dispatchEvent(new CustomEvent("step-success", { bubbles: true, composed: true, detail: this.result }));
		} catch (err) {
