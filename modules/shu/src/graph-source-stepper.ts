/**
 * GraphSourceStepper — the graph a page reads, as the steps that answer those reads.
 *
 * A page reads the site's graph the same way whatever is under it: a query, a count over a span, one individual with
 * its edges, the clustered sample a graph view draws, and the reads a federated peer answers. Each answers from the
 * store this instance holds, and each is declared `fallback`, so a consumer with an engine of its own answers instead
 * and the page asks no differently.
 *
 * These steps were the monitor's by history rather than by concern: the monitor is a view of a run, and the graph is
 * what every view reads from.
 */
import { z } from "zod";
import { AStepper, type TStepperSteps } from "@haibun/core/lib/astepper.js";
import { AccessLevelSchema, AccessQueryLevelSchema, storeScopeFor } from "@haibun/core/lib/resources.js";
...
import { queryQuadStore, individualWithEdges } from "@haibun/core/lib/quad-store.js";
import { hypermediaDomainMap } from "@haibun/core/lib/domains.js";
import { relOf } from "@haibun/core/lib/hypermedia.js";
import { LinkRelations, type TPropertyDef } from "@haibun/core/lib/resources.js";
...
import { buildResourceRels } from "@haibun/core/lib/hypermedia.js";
...
/** An individual with its edges, as a view reads it. */
const IndividualWithEdgesSchema = z.object({
	vertex: z.record(z.string(), z.unknown()),
	edges: z.array(z.object({ type: z.string(), direction: z.enum(["out", "in"]), target: z.record(z.string(), z.unknown()) })),
...
export default class GraphSourceStepper extends AStepper {
	...
	steps = {
		getIndividualWithEdges: {
			...
			// A page reads an individual with its edges the same way whatever is under it. This answers from the store
			// this instance holds, which is what a site with no graph engine of its own has; a deployment that gates a
			// read by what a reader may see registers its own step, and the page reads through that one instead.
		...
		getSelectValues: {
			...
			action: async ({ label }: { label: string }) => {
				...
				for (const [property, definition] of Object.entries(domain.topology.properties)) {
					if (relOf(definition as TPropertyDef) === LinkRelations.CONTEXT.rel) values[property] = await store.distinctPropertyValues(label, property);
				}
