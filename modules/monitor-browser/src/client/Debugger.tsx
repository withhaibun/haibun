...
export function Debugger({ prompt, onSubmit, className }: DebuggerProps & { className?: string }) {
	...
	return (
		<div
			className={`bg-popover text-popover-foreground border border-border/40 rounded-sm shadow-md p-3 w-full h-full flex flex-col ${className}`}
		...
		>
			...
			<div className="bg-muted/50 p-2 rounded-sm text-xs font-mono mb-3 break-all border border-border/20">{prompt.message}</div>
