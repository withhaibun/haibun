...
describe("generateSequenceDiagramFromTraces", () => {
	...
	it("generates basic sequence diagram with request", () => {
		const traces = [createTrace("request", { method: "GET", requestingURL: "http://api.example.com/users" })];
	...
	it("handles route events as requests", () => {
		const traces = [createTrace("route", { method: "GET", requestingURL: "http://api.example.com/resource" })];
	...
	it("sanitizes participant names", () => {
		const traces = [createTrace("request", { requestingURL: "http://api-v2.example.com:8080/test" })];
