 ...
 */
export function QuadGraphDiagram({ quads, currentTime, startTime = 0, namedGraphFilter }: QuadGraphDiagramProps) {
	...
	const universeQuads = useMemo(() => {
		...
		if (namedGraphFilter && namedGraphFilter.length > 0) {
			u = quads.filter((q) => !q.namedGraph || namedGraphFilter.includes(q.namedGraph));
		}
				...
				<div className="flex items-center gap-2 text-xs border-r border-slate-200 pr-4 h-full">
					...
					{Object.keys(contextTree).map((parent) => (
						<label key={parent} className="flex items-center gap-1 cursor-pointer select-none whitespace-nowrap">
							...
							{contextTree[parent].length > 0 && <span className="text-slate-400 text-[10px]">({contextTree[parent].length})</span>}
						</label>
					))}
				</div>
	...
	return (
		<ArtifactFrame title="Quad Graph" toolbar={toolbar} onCopy={handleCopy} className="quad-graph-diagram">
...
export function generateGraphFromQuads(quads: TQuad[], namedGraphFilter?: string[]): string {
	...
	if (namedGraphFilter && namedGraphFilter.length > 0) {
		filteredQuads = filteredQuads.filter((q) => !q.namedGraph || namedGraphFilter.includes(q.namedGraph));
	}
