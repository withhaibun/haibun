...
function App() {
	...
	type TStepIndex = Record<
		string,
		{
			in: string;
			seqPath: number[];
			source?: { path?: string; lineNumber?: number };
			action?: { actionName?: string; stepperName?: string };
		}
	>;
				...
				if (e.kind === "hidden-block") {
					...
					return (
						<div key={block.id} className="flex justify-start my-1 px-2">
							...
							>
								<span>
									~~~ {block.count} {typesStr} hidden ({reasonStr}) ~~~
								</span>
							</button>
						</div>
					);
	...
	const enrichedSelectedEvent = useMemo(() => {
		...
		if (showQuadGraph) {
			...
			const isQuadArtifact = (e: THaibunEvent): e is TQuadArtifact => e.kind === "artifact" && e.artifactType === "json" && !!(e as Record<string, unknown>).json;
			...
			<main className="w-full pt-20 px-4 max-w-[1800px] mx-auto" data-testid={TEST_IDS.APP.MAIN}>
				<div className="flex gap-4">
					<div className="flex-1 min-w-0">
						...
						{viewMode === "raw" && (
							<pre data-testid={TEST_IDS.VIEWS.RAW} className="text-xs p-4 overflow-auto">
