...
describe("Timeline", () => {
	describe("calculateTimelineBounds", () => {
		...
		it("throws when no events have timestamps", () => {
			const events = [{ id: "1", kind: "log" } as unknown as THaibunEvent, { id: "2", kind: "log" } as unknown as THaibunEvent];
		...
		it("handles single event", () => {
			const events: THaibunEvent[] = [{ id: "1", kind: "log", timestamp: 5000, level: "info", message: "only" } as THaibunEvent];
			const result = calculateTimelineBounds(events);
