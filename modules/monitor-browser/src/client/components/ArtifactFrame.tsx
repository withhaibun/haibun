  ...
  return (
    ...
    >
      <div className="flex justify-between items-center p-2 bg-white border-b border-slate-300 shrink-0 gap-4 h-10">
        <div className="font-bold text-sm text-slate-700 shrink-0 truncate" title={title}>{title}</div>
      ...
      <div className={`flex-1 overflow-auto relative ${contentClassName} ${isFullscreen ? '' : 'max-h-full'}`}>
        {children}
      </div>
    </div>
  );
