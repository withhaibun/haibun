...
export class Runner {
	...
	async runFeaturesAndBackgrounds(
		csteppers: CStepper[],
		featuresBackgrounds: TFeaturesBackgrounds,
	): Promise<TExecutorResult> {
		const phaseRunner = new PhaseRunner(this.world);

		try {
			this.steppers = await phaseRunner.tryPhase("Steppers", () =>
				createSteppers(csteppers),
			);
			phaseRunner.steppers = this.steppers;

			await phaseRunner.tryPhase("WorldsAndDomains", () =>
				setStepperWorldsAndDomains(this.steppers, this.world),
			);

			// Collect domain concerns before Expand so domains are available during resolution
			await phaseRunner.tryPhase("Concerns", () =>
				addStepperConcerns(this.world, this.steppers),
			);

			// Auto-suppress NDJSON output if any monitor stepper is configured
			await phaseRunner.tryPhase("Options", () => {
				if (
					this.steppers.some((s) => s.kind === StepperKinds.MONITOR) &&
					this.world.eventLogger
				) {
			...
			const expandedFeatures = await phaseRunner.tryPhase("Expand", () =>
				expand(featuresBackgrounds),
			);

			const resolver = new Resolver(
				this.steppers,
				featuresBackgrounds.backgrounds,
			);
			const resolvedFeatures = await phaseRunner.tryPhase("Resolve", () =>
				resolver.resolveStepsFromFeatures(expandedFeatures),
			);

			return await phaseRunner.tryPhase("Execute", () =>
				Executor.executeFeatures(this.steppers, this.world, resolvedFeatures),
			);
		} catch (error) {
