...
import { interpolate, resolveVariable } from './variables.js';
...
describe('variables integration', () => {
    ...
    it('interpolates env variables', async () => {
        const feature = { path: '/features/test.feature', content: '' };
        const res = await passWithDefaults([feature], steppers, { options: { DEST: 'default', envVariables: { NAME: 'Env' } }, moduleOptions: {} });
        expect(res.ok).toBe(true);
        const world = res.world;
