...
describe("what a run depends on", () => {
	...
	it("derives the bases, the module of every stepper named, and the paths the configuration adds, each once", () => {
		...
		const core = roots.filter((r) => nodeFS.existsSync(path.join(r, "package.json")) && JSON.parse(nodeFS.readFileSync(path.join(r, "package.json"), "utf-8")).name === "@haibun/core");
		expect(core.length, "the core once, for its two steppers").toBe(1);
