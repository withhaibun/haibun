...
export function asHistoryWithMeta(
	...
	const feature =
		logFeature?.messageContext?.tag?.params?.feature || 'missing feature' + (logFeature?.messageContext?.tag || logFeature);
...
export function asArtifact(logHistory: TLogHistory | undefined): TLogHistoryWithArtifact | undefined {
	return logHistory !== undefined && (<TLogHistoryWithArtifact>logHistory)?.messageContext?.artifact
		? <TLogHistoryWithArtifact>logHistory
		: undefined;
}

export function asStepperActionType(
	logHistory: TLogHistory | undefined,
	stepperType: string
): TLogHistoryWithExecutorTopic | undefined {
