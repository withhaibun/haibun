...
type TActionsFromStepper<S extends TStepperSteps> = {
	[K in keyof S]: S[K] extends { gwta: infer G } ? (G extends string ? TCurriedAction<G> : never) : never;
};
...
/**
 * Generate kireji action factories for vertex CRUD from a label and domain key.
 * No stepper instance needed — works at import time.
 * The actual handlers are generated by the registry from domain definitions at runtime.
 */
export const withVertexActions = (label: string, domainKey: string) => {
	...
	return Object.fromEntries([
		...
		makeAction(`list${label}s`, `list ${lc}s`, []),
	]);
};
