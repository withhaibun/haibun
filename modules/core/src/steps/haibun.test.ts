...
Prose sections are indicated by the presence of punctuation at the end of paragraphs.`,
	...
	it('process effect callback', async () => {
		const feature = { path: '/features/test.feature', content: 'after every TestSteps, passes\nhave a test' };
		const result = await testWithDefaults([feature], [Haibun, TestSteps]);
		expect(result.ok).toBe(true);
		expect(result.featureResults![0].stepResults.length).toBe(3);
		expect(result.featureResults![0].stepResults[2].seq).toBe(2.1);
	});
	it.only('process multiple effect callbacks', async () => {
		...
		expect(result.featureResults![0].stepResults[0].seq).toBe(1);
		expect(result.featureResults![0].stepResults[1].seq).toBe(2);
		expect(result.featureResults![0].stepResults[2].seq).toBe(3);
		expect(result.featureResults![0].stepResults[3].seq).toBe(3.1);
	});
});
