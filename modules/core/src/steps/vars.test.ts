...
describe('vars', () => {
  ...
  it('empty does not overwrite', async () => {
    const { world, vstep } = await getTestEnv(['vars'], 'set empty "x" to newValue', { ...getDefaultWorld().world, shared: new WorldContext('test vars empty does not overwrite', { x: 'notY' }) });
