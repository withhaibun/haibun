...
/** Initialize runtime state once at execution start. */
...
/** Reset per-feature runtime state. Called once before each feature runs. */
...
export class Executor {
	...
	static async action(
		...
		const stepUsage =
			(world.runtime.observations.get("stepUsage") as Map<string, number>) ||
	...
	static async executeFeatures(
		...
		for (const feature of features) {
			...
			const newWorld = {
				...world,
				tag: { ...world.tag, featureNum: featureNum, featureName },
			};
			...
			world.eventLogger.info(
				`feature ${featureNum}/${features.length}: ${feature.path}`,
			);
			...
			if (!thisFeatureOK) {
				...
				await doStepperCycle(steppers, "onFailure", {
					featureResult,
					failedStep,
				});
			}
...
export class FeatureExecutor {
	...
	async doFeature(feature: TResolvedFeature): Promise<TFeatureResult> {
		...
		const featureResult: TFeatureResult = {
			path: feature.path,
			ok,
			stepResults: world.runtime.stepResults,
		};

		return featureResult;
	}

	static async doFeatureStep(
		steppers: AStepper[],
		featureStep: TFeatureStep,
		world: TWorld,
		...
		if (world.runtime.exhaustionError) {
			return createFailedStepResult(
				`Execution halted: ${world.runtime.exhaustionError}`,
			);
		}
		...
		if (isFullCycles) {
			if (
				action.actionName !== FEATURE_START &&
				action.actionName !== SCENARIO_START
			) {
			...
			let doAction = true;
			while (doAction) {
				await doStepperCycle(steppers, "beforeStep", <TBeforeStep>{
					featureStep,
				});
				actionResult = await Executor.action(
					steppers,
					featureStep,
					action,
					args,
				...
				if (
					...
					const errorMessage = !actionResult.ok
						? (actionResult as TNotOKActionResult).message
						: undefined;
					const products =
						"products" in actionResult
							? (actionResult.products as Record<string, unknown>)
							: undefined;
		...
		const stepResult = stepResultFromActionResult(
			actionResult,
			action,
			start,
			Timer.since(),
...
const doStepperCycle = async <K extends keyof IStepperCycles>(
	...
	for (const cycling of hasCycles) {
		...
		if (cycle) {
			...
			const result = await (
				cycle as (...a: unknown[]) => Promise<unknown>
			).apply(cycling, paramsForApply);
			results.push(
				result as Awaited<ReturnType<NonNullable<IStepperCycles[K]>>>,
			);
		}
	}
	return results;
...
export const addStepperConcerns = async (
	world: TWorld,
	steppers: AStepper[],
) => {
...
export function incSeqPath(
	...
	let found = true;
	while (found) {
		found = withSeqPath.some(
			(r) => JSON.stringify(r.seqPath) === JSON.stringify(candidate),
		);
		if (found) {
			last += dir;
