...
import { flattenTestIds } from "@haibun/core/lib/test/lib.js";
...
const filterIds = (groups: string[]) => {
	return flattenTestIds(Object.fromEntries(Object.entries(TEST_IDS).filter(([k]) => groups.includes(k))));
};

const testableIds = filterIds(testableGroups).filter((id) => !(conditionalIds as string[]).includes(id));

// Create domains
const defineDomain = (name: string, ids: string[]) => `set of ${name} is [${ids.map((id) => `"${id}"`).join(" ")}]`;
