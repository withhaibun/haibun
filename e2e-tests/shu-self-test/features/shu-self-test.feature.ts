...
export const features: TKirejiExport = {
	"Shu SPA Self-Test": [
		...
		"A step run to carry another step out reports under it, so a reader reading what the feature did is not shown the machinery. Asking for those steps shows them, and each names the step it was run to carry out, which a reader reads that step from. The screenshot this run takes after every browser step is such a step, so asking for them shows it.",
