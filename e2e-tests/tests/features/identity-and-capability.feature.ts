...
export const features: TKirejiExport = {
	"Identity and capability authorization": [
		...
		'issue session grant for token "rpc-ranger-token" with action "TestServer:protected"',
		...
		'revoke session grant for token "rpc-ranger-token"',
		...
		`Nothing about the capability is transport-specific. The MCP endpoint, the tool interface an external agent uses, admits a configured client token to connect, then reads that same token through the same resolver and feeds it into the same gate, so a host-side grant or revocation flips authorization identically here. The client first connects and finds the protected tool in the tool list and among the steps shown for a pattern, neither of which is authority, then the same grant, least-privilege, and revoke lifecycle plays out over the tool call. Grant and revocation take effect only when issued against, and read from, the same running authority: a token authorised against a context that never saw the grant is still refused.`,
		"serve mcp tools at /mcp",
		'webserver is listening for "capability over mcp"',
		'mcp tools at "http://localhost:8138/mcp" include "TestServer-protectedRpcPing" when bearer token is "mcp-client-token"',
		'mcp steps shown at "http://localhost:8138/mcp" matching "^TestServer-protected" include "TestServer-protectedRpcPing" when bearer token is "mcp-client-token"',
		...
		'issue session grant for token "mcp-client-token" with action "TestServer:protected"',
		...
		'revoke session grant for token "mcp-client-token"',
		'mcp call to "http://localhost:8138/mcp" with tool "TestServer-protectedRpcPing" is denied when bearer token is "mcp-client-token"',
	],
};
