...
export const features: TKirejiExport = {
	"RPC capability dispatch": [
		feature({ feature: "RPC capability dispatch" }),
		...
		scenario({ scenario: "Protected RPC can be granted and revoked through a session grant" }),
		...
		The transport does not change. The step definition does not change. What changes is the authority state: the grant is issued, the bearer token works, the grant is revoked, and the same token stops working.`,
		"enable rpc",
		'webserver is listening for "rpc capability e2e"',
		'issue session grant for token "session-token" with action "TestServer:protected"',
		'rpc call to "http://localhost:8123/rpc/TestServer-protectedRpcPing" with method "TestServer-protectedRpcPing" succeeds when bearer token is "session-token"',
		'revoke session grant for token "session-token"',
		'rpc call to "http://localhost:8123/rpc/TestServer-protectedRpcPing" with method "TestServer-protectedRpcPing" is denied when bearer token is "session-token"',
	],
};
