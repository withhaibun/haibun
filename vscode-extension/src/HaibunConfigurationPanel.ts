...
export class HaibunConfigurationTreeProvider implements vscode.TreeDataProvider<TreeNode> {
	...
	private _getOrCreateTreeNode(node: DirNode, parent?: TreeNode): TreeNode {
		...
		if (node.isDir) {
			treeNode = new TreeNode("folder", node.name, "", vscode.TreeItemCollapsibleState.Collapsed);
			treeNode.dirNode = node;
		} else {
			// Check if file is dirty (has unsaved changes)
			const uri = vscode.Uri.file(node.path);
			const isDirty = vscode.workspace.textDocuments.some((doc) => doc.uri.fsPath === node.path && doc.isDirty);

			treeNode = new TreeNode("file", isDirty ? `● ${node.name}` : node.name, "", vscode.TreeItemCollapsibleState.None, {
				command: "vscode.open",
				title: "Open File",
				arguments: [uri],
			});
	...
	private _discoverFiles(): void {
		...
		const effectiveCwd = userCwd ? (path.isAbsolute(userCwd) ? userCwd : path.resolve(workspaceRoot, userCwd)) : workspaceRoot;
	...
	getChildren(element?: TreeNode): Thenable<TreeNode[]> {
		if (element) {
			...
			if (element.nodeType === "mcp-tool" && element.toolData) {
				...
				for (const [name, def] of Object.entries(properties)) {
					...
					const node = new TreeNode("mcp-tool-param", name, value, vscode.TreeItemCollapsibleState.None, {
						command: "haibun.setMcpToolParam",
						title: "Set Parameter",
						arguments: [tool.name, name],
					});
					node.toolData = tool;
			...
			if (element.commonConfig) {
				...
				return Promise.resolve([
					new TreeNode("config", "Working Directory", cwd || "(workspace root)", vscode.TreeItemCollapsibleState.None, {
			...
			if (element.mcpConfig) {
				...
				const items: TreeNode[] = [
					new TreeNode("config", "Enabled", mcpEnabled ? "Yes" : "No", vscode.TreeItemCollapsibleState.None, {
						command: "haibun.toggleMcp",
						title: "Toggle MCP",
					}),
				];
				if (mcpEnabled) {
					items.push(
						new TreeNode("config", "Port", String(mcpPort), vscode.TreeItemCollapsibleState.None, {
		...
		// 1. Show Version info
		if (this._version) {
			items.push(new TreeNode("info", "Haibun Extension", `v${this._version}`, vscode.TreeItemCollapsibleState.None));
		}

		// 2. Show error prominently if present
		if (this._errorMessage) {
			const errorNode = new TreeNode("error", this._errorMessage, this._errorAction, vscode.TreeItemCollapsibleState.None, {
		...
		const configNode = new TreeNode("section", "Configuration", "", vscode.TreeItemCollapsibleState.Expanded);
		...
		if (this._mcpTools.length > 0) {
			const toolsNode = new TreeNode("section", "MCP Tools", `${this._mcpTools.length}`, vscode.TreeItemCollapsibleState.Collapsed);
		...
		switch (nodeType) {
			...
			case "mcp-tool-output":
				this.iconPath = new vscode.ThemeIcon(
					this.toolOutput?.isRunning ? "sync~spin" : this.toolOutput?.isError ? "error" : "output",
